...
class Machine:
    ...
    def set_joltage(self):
        def set_joltage_rec(remaining_button_idxs, seq, remaining_joltage):
#            print("seq is %s"%seq)
...
#            print("solution %s"%self.__joltage_solution)
            ...
            new_remaining_button_idxs = remaining_button_idxs[:]
            random.shuffle(new_remaining_button_idxs)

            for idxidx, idx in enumerate(reversed(new_remaining_button_idxs)):
...
#                print("Apply button %s, %s"%(idx, button))
                ...
                if len(new_seq) < self.__joltage_min_presses_count_upperbound - 1:
                    set_joltage_rec(new_remaining_button_idxs[:len(new_remaining_button_idxs)-idxidx], new_seq, new_remaining_joltage)

        #Set some initial upperbound
        self.__joltage_min_presses_count_upperbound = sum(self.joltage_requirements) + 1

        #Start recursion
        set_joltage_rec(list(range(len(self.buttons))), [], self.joltage_requirements)
